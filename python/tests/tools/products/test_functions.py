...
@pytest.mark.asyncio
async def test_list_products_with_id_filter(mock_api, admin_context):
    ...
    assert 'id="prod-1"' in query.get("where", "")
...
async def test_create_product_calls_post(mock_api, admin_context):
    params = CreateProductParams(
        name=LocalizedString(en="Widget"),
        product_type_id="pt-1",
        slug=LocalizedString(en="widget"),
    )
...
async def test_create_product_raises_context_error_without_admin(mock_api):
    ...
    with pytest.raises(ContextError, match="create_product"):
        await create_product(params, mock_api, ctx)
