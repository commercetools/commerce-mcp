...
The server identity reported `version: '0.4.0'` while the package was at 4.x. It now reports the real version, alongside a `description`, `title`, `websiteUrl` and usage `instructions`. A test asserts the constant matches `package.json`, so the two cannot drift again.
